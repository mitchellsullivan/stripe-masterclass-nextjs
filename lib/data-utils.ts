export const getAllProducts = (): ShopProduct[] => {
  return SHOP_DATA;
}

export const getProductById = (id: number): ShopProduct => {
  return SHOP_DATA.find(p => p.id === id);
}

export const getFeaturedProducts = (): ShopProduct[] => {
  return SHOP_DATA.filter((product, i) => i < 4);
}
